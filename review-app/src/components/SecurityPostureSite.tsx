                  ...
                  <div className="timeline-list">
                    {recentActivityFeed.map((item, index) => (
                      <SurfaceTimelineItem
                        ...
                        description={`${item.geography_bucket} · ${item.session_label} · ${formatRelativeAgeFromIso(item.recorded_at_utc)}`}
                        eyebrow={formatUtcDateTimeLabel(item.recorded_at_utc)}
                        key={`${item.recorded_at_utc}:${item.session_label}:${item.route}:${index}`}
                        markerState={index === 0 ? "active" : "complete"}
                        title={`${formatRouteLabel(item.route)} route activity`}
                      />
                    ))}
                  </div>
                ) : (
                ...
                {uptimeHistory.length > 0 ? (
                  <div className="timeline-list">
                    {uptimeHistory.map((item, index) => (
                      <SurfaceTimelineItem
                        badge={
                          <StatusBadge tone={getHealthCheckTone(item.overall_ok)}>
                            {item.overall_ok ? "Healthy" : "Degraded"}
                          </StatusBadge>
                        }
                        description={item.note}
                        eyebrow={formatUtcDateTimeLabel(item.checked_at_utc)}
                        key={`${item.checked_at_utc}:${index}`}
                        markerState={getHealthCheckMarkerState(item, index)}
                        title={index === 0 ? "Latest external probe" : "External probe result"}
                      />
                    ))}
                  </div>
                ) : (
